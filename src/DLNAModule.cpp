...
{
	Log(LEVEL_INFO, "Starting DLNAModule-%s-%.8s, built at %s", DLNA_VERSION_BRANCH, DLNA_VERSION_COMMIT, BUILD_TIMESTAMP);
	StartDiscover();
...
void DLNAModule::TaskThread()
{
	...
	while (isDLNAModuleRunning)
	{
		...
		if (pTask != nullptr)
		{
			...
			{
				Log(LEVEL_INFO, "BrowseRequest: ObjID=%s, name=%s, location=%s", pTask->objid, it->second.friendlyName.c_str(), it->second.location.c_str());
